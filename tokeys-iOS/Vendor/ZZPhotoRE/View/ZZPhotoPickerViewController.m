...
#import "ZZPhotoHud.h"
@interface ZZPhotoPickerViewController ()<UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout,ZZBrowserPickerDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate>
...
@property (strong, nonatomic) UIButton *cameraBtn;                    //拍照按钮
...
-(UIButton *)doneBtn{
    if (!_doneBtn) {
        ...
        [_doneBtn setTitleColor:blcolor forState:UIControlStateNormal];
    }
    return _doneBtn;
}
...
- (UIButton *)cameraBtn{
    if (!_cameraBtn) {
        _cameraBtn = [[UIButton alloc]initWithFrame:CGRectMake((ZZ_VW-30)/2.0, 7, 40, 34)];
...
-(UIButton *)previewBtn{
    if (!_previewBtn) {
        _previewBtn = [[UIButton alloc]initWithFrame:CGRectMake(10, 10, 40, 25)];
        [_previewBtn addTarget:self action:@selector(preview) forControlEvents:UIControlEventTouchUpInside];
        //[_previewBtn setImage:[UIImage imageNamed:@"关闭"] forState:UIControlStateNormal];
        ...
        [_previewBtn setTitle:@"关闭" forState:UIControlStateNormal];
        [_previewBtn setTitleColor:blcolor forState:UIControlStateNormal];
    }
    return _previewBtn;
}
...
-(void)setUpTabbar
{
    ...
    [view addSubview:self.cameraBtn];
