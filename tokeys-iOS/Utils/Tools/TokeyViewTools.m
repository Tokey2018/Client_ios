...

{
    ...
    [btn.titleLabel setFont:[UIFont systemFontOfSize:W_In_375(15)]];
    if (rect.size.width) {
        btn.frame = rect;
    }
    if (title) {
...
+(UILabel *)createHeiLabelTextWithView:(UILabel *)label andwithChangeColorTxt:(NSString *)string withColor:(UIColor *)color withFont:(UIFont*)font{
    ...
    // 行间距设置为30
    [paragraphStyle  setLineSpacing:W_In_375(8)];
    ...
    if(authorizationStatus ==CNAuthorizationStatusNotDetermined) {
        ...
        [contactStore requestAccessForEntityType:CNEntityTypeContacts completionHandler:^(BOOL granted,NSError*_Nullable error) {
            ...
            if(granted) {
                ...
                //  contactArr = [self getContact]; //5.获取用户通讯录
    ...
    if(authorizationStatus ==CNAuthorizationStatusAuthorized) {
        ...
        [contactStore enumerateContactsWithFetchRequest:fetchRequest error:nil usingBlock:^(CNContact*_Nonnull contact,BOOL*_Nonnull stop) {
            ...
            for(CNLabeledValue*labelValue in phoneNumbers) {
                ...
                if(phoneNumber.stringValue.length==11){
                    NSDictionary*contact =@{@"phone":phoneNumber.stringValue,@"name":[NSString stringWithFormat:@"%@ %@", familyName, givenName]};
